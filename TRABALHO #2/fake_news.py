...
def palavras_chave():
    ...
    palavra1 = input("Digite a 1ª palavra-chave: ").upper()
    palavra2 = input("Digite a 2ª palavra-chave: ").upper()
    
    autores = set([autor['author'] for autor in fakeNews if palavra1 in autor['text'].upper() and palavra2 in autor['text'].upper()])

    autores2 = sorted(list(autores))

    if len(autores2) == 0:
        print(f"Nenhum autor escreveu sobre AMBAS as palavras: '{palavra1}' e '{palavra2}'")
    else:
        print(f"\nAutores que escreveram sobre AMBAS as palavras '{palavra1}' e '{palavra2}': {', '.join(autores2)}")
